...
# --- Import Handling ---
try:
    from timm.layers import resample_abs_pos_embed, to_2tuple, trunc_normal_
except ImportError:
    ...
    def resample_abs_pos_embed(posemb, new_size, old_size, num_prefix_tokens=0, verbose=False):
        return F.interpolate(posemb.permute(0, 2, 1), size=new_size[0] * new_size[1], mode="linear").permute(0, 2, 1)


# --- Components ---
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4.0, qkv_bias=False, qk_scale=None, norm_layer=nn.LayerNorm, skip=False, use_checkpoint=False):
        super().__init__()
...
class PatchTokenizer(nn.Module):
    ...
    def forward(self, images):
        ...
        for idx in range(self.num_scales):
            ...
            if idx > 0:
                constituent = einops.rearrange(images, "b c (h n1 p) (w n2 q) -> b h w (n1 n2) c p q", p=self.base_patch_size, q=self.base_patch_size, n1=ps // self.base_patch_size, n2=ps // self.base_patch_size)
...
class TokenizedZeroConvPatchAttn(nn.Module):
    def __init__(self, image_size=224, patch_size=16, embed_dim=768, num_scales=2):
        ...
        self.base_mini_pos_embed = nn.Parameter(torch.randn(1, 4, embed_dim) * 0.02)
    ...
    def forward(self, x, base_pos_embed, input_dict):
        ...
        for idx, ps in enumerate(self.patch_sizes[1:]):
            ...
            mini_pos = mini_pos.view(1, n, n, self.embed_dim)

            full = (full + mini_pos).permute(0, 3, 1, 2)

            for _ in range(idx + 1):
                full = self.patch_attn(full)
        ...
        valid_indices = output_mask != -1
        expanded_outputs = expanded_outputs[valid_indices]

        # Vectorized Padding
        seqlens = input_dict["seqlens"]
        split_tokens = torch.split(expanded_outputs, seqlens)
        padded_batch = pad_sequence(split_tokens, batch_first=True)
